...
ax_accel= fb_x/P.mass + g*sin(theta) + accel_noise_xyz(1); % m/s^2
ay_accel= fb_y/P.mass - g*cos(theta)*sin(phi) + accel_noise_xyz(2); % m/s^2
az_accel= fb_z/P.mass - g*cos(theta)*cos(phi) + accel_noise_xyz(3); % m/s^2
...
true_static_press = P0*exp(-M*P.gravity/R/T*(-pd)); % True static pressure at UAV altitude (above sea level), N/m^2
static_press = true_static_press + bias_static_press + randn*P.sigma_noise_static_press; % Measured static pressure, N/m^2
...
diff_press = true_diff_press + bias_diff_press + randn*P.sigma_noise_diff_press; % Measured differential pressure, N/m^2
...
psi_mag= psi + bias_mag + randn*P.sigma_noise_mag; % Magnetometer measurement, rad
...
if(time>time_gps_prev+P.Ts_gps)
    ...
    R_b2ned = R_ned2b';
    v = R_b2ned*[u;v;w];
    Vn_gps = v(1) + randn*P.sigma_noise_gps_speed;
    Ve_gps = v(2) + randn*P.sigma_noise_gps_speed;
    Vd_gps = v(3) + randn*P.sigma_noise_gps_speed;
